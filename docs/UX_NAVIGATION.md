...
```mermaid
flowchart TD
    H["Домой: что делать сейчас?"] --> N["Один приоритетный шаг"]
    H --> L["Учиться"]
    L --> A["Мои занятия: текущие и на паузе"]
    L --> C["Подготовиться к разговору"]
    C --> S["Короткая репетиция"]
    C --> M["Разговор с поворотами"]
    L --> V["Повторы и слабые места"]
    H --> T["Помощник: своя фраза"]
    H --> P["Профиль"]
    P --> G["Языки и уровень"]
    P --> R["Напоминания"]
    P --> D["Данные и приватность"]
    T --> E["Мои ошибки"]
    T --> K["Карточки"]
...
- Нижняя клавиатура v6 содержит только `Сегодня`, `Учиться`, `Помощник`,
  `Профиль`. Она не содержит форматы упражнений и ответы: первые выбираются
  после понятной цели, вторые принадлежат карточке.
...
- `Домой` всегда означает dashboard. Возврат к родителю называет назначение:
  `К профилю`, `К языкам`, `К помощнику`.
...
- Новый учебный формат входит в `Учиться` или `Помощник`; новый пункт верхнего
...
1. подготовиться к разговору, который предстоит завтра;
2. найти и пройти двухминутную практику по текущей теме;
3. отправить свою фразу, проверить её и вернуться в сохранённое занятие.
...
Вариативный разговор открывается после `Учиться → Подготовиться к разговору`,
а не рядом с другими внутренними форматами. Он визуально оформлен как дело:
номер шага, не более трёх накопленных фактов, реплика

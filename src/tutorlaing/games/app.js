...
function render(message = "", isError = false) {
  ...
  app.querySelectorAll("[data-cancel-link]").forEach((button) => button.addEventListener("click", () => cancelLink(button.dataset.cancelLink || "")));
...
async function cancelLink(token) { try { await api("/games/api/cancel-link", {token}); await refresh("Ссылка закрыта и больше не сработает."); } catch (error) { render(error.message, true); } }

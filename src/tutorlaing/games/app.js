...
function profileView() {
  ...
  return `<section class="panel"><h2>Ваш игровой ник</h2><p>По нему вас смогут найти только другие пользователи игр этого бота.</p><form class="stack" data-form="profile"><label>Ник без @<input name="nickname" maxlength="24" autocomplete="off" placeholder="например, ola_krakow" required></label><button class="button">Сохранить ник</button></form></section>`;
}
function lobbyView() {
  if (!model.profile) return "";
  ...
  return `<section class="panel"><h2>Новая партия</h2><p>Выберите игру и введите игровой ник соперника.</p><div class="game-grid">${tiles}</div><form class="stack" data-form="invite"><label>Ник соперника<input name="nickname" maxlength="24" autocomplete="off" placeholder="ник без @" required></label><button class="button">Отправить приглашение</button></form></section>${games ? `<section class="panel"><h2>Ваши партии</h2><div class="game-list">${games}</div></section>` : ""}`;
}
...
function render(message = "", isError = false) {
  ...
  app.innerHTML = `<header class="masthead"><div><div class="eyebrow">Tutorlaing · вдвоём</div><h1>Игровой стол</h1></div>${model?.profile ? `<span class="nick">@${escapeHtml(model.profile.nickname)}</span>` : ""}</header>${renderNotice(message, isError)}${profileView()}${boardView(game)}${lobbyView()}`;
  ...
  app.querySelectorAll("[data-action]").forEach((button) => button.addEventListener("click", () => { const action = button.dataset.action; if (action === "lobby") { selectedId = null; render(); } else perform(`/games/api/${action}`, {game_id: currentGame().id}); }));
  const profile = app.querySelector('[data-form="profile"]');
  if (profile) profile.addEventListener("submit", (event) => { event.preventDefault(); perform("/games/api/profile", {nickname: new FormData(profile).get("nickname")}); });
  const invite = app.querySelector('[data-form="invite"]');
  if (invite) invite.addEventListener("submit", (event) => { event.preventDefault(); perform("/games/api/invitations", {kind: selectedGame, nickname: new FormData(invite).get("nickname")}); });
}
...
refresh();
polling = window.setInterval(() => { if (document.visibilityState === "visible" && model?.profile) refresh(); }, 3000);
